...
export class UserService {
  ...
  addenseignant (username: string, password: string, email: string): Observable<any> {
    return this.http.post(`${this.apiUrl}/addEnseignant`, { username, password, email }).pipe(
      catchError(this.handleError)
    );
  }
  getEnseign(): Observable<Users[]> {
    return this.http.get<Users[]>(`${this.apiUrl}/enseignants`).pipe(
      catchError((error: HttpErrorResponse) => {
        this.handleHttpError(error);
  ...
  deleteens(ensid: string): Observable<any> {
    return this.http.delete<any>(`${this.apiUrl}/deleteEnseignant/${ensid}`).pipe(
      catchError(this.handleError)
    );
  }


  updateens(id: string ,username: string, password: string, email: string): Observable<any> {
    return this.http.put(`${this.apiUrl}/updateEnseignant/${id}`,{username,password,email}).pipe(
      catchError(this.handleError)
    );
  }
